...
#include "K2Node_BaseAsyncConstructObject.h"
...
#define LOCTEXT_NAMESPACE "UK2Node_BaseAsyncConstructObject"

UK2Node_BaseAsyncConstructObject::UK2Node_BaseAsyncConstructObject(const FObjectInitializer& ObjectInitializer)
...
{
    NodeTooltip = LOCTEXT("NodeTooltip", "Constructs an Async Object");
}

// UEdGraphNode interface

void UK2Node_BaseAsyncConstructObject::AllocateDefaultPins()
{
...
FText UK2Node_BaseAsyncConstructObject::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
    if (GetFactoryFunction() == nullptr)
    {
        return FText(LOCTEXT("UK2Node_BaseAsyncConstructObjectGetNodeTitle", "Async Task: Missing Function"));
    }
...
FText UK2Node_BaseAsyncConstructObject::GetTooltipText() const
{
...
bool UK2Node_BaseAsyncConstructObject::IsCompatibleWithGraph(const UEdGraph* TargetGraph) const
{
...
void UK2Node_BaseAsyncConstructObject::ValidateNodeDuringCompilation(class FCompilerResultsLog& MessageLog) const
{
...
void UK2Node_BaseAsyncConstructObject::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	if (CallCreateProxyObjectNode->GetTargetFunction() == nullptr)
	{
		...
		const FString RawMessage = LOCTEXT("AsyncTaskError", "BaseAsyncConstructObject: Missing function %s from class %s for async task @@").ToString();
	...
	{
		if (FHelper::ValidDataPin(CurrentPin, EGPD_Input, Schema))
		{
	...
	UEdGraphPin* OutputAsyncTaskProxy = FindPin(FHelper::GetAsyncTaskProxyName());
	...
	for (auto CurrentPin : Pins)
	{
		if ((OutputAsyncTaskProxy != CurrentPin) && FHelper::ValidDataPin(CurrentPin, EGPD_Output, Schema))
		{
	...
	{
		bIsErrorFree &= FHelper::HandleDelegateImplementation(*PropertyIt, VariableOutputs, ProxyObjectPin, LastThenPin, this, SourceGraph, CompilerContext);
	}

	if (CallCreateProxyObjectNode->FindPinChecked(Schema->PN_Then) == LastThenPin)
	{
		CompilerContext.MessageLog.Error(*LOCTEXT("MissingDelegateProperties", "BaseAsyncConstructObject: Proxy has no delegates defined. @@").ToString(), this);
		return;
	}
	...
	if (!bIsErrorFree)
	{
		CompilerContext.MessageLog.Error(*LOCTEXT("InternalConnectionError", "BaseAsyncConstructObject: Internal connection error. @@").ToString(), this);
	}
...
bool UK2Node_BaseAsyncConstructObject::HasExternalDependencies(TArray<class UStruct*>* OptionalOutput) const
{
...
FName UK2Node_BaseAsyncConstructObject::GetCornerIcon() const
{
	return TEXT("Graph.Latent.LatentIcon");
}

void UK2Node_BaseAsyncConstructObject::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UK2Node_BaseAsyncConstructObject::GetMenuCategory() const
{	
...
UK2Node::ERedirectType UK2Node_BaseAsyncConstructObject::DoPinsMatchForReconstruction(const UEdGraphPin* NewPin, int32 NewPinIndex, const UEdGraphPin* OldPin, int32 OldPinIndex) const
{
...
UFunction* UK2Node_BaseAsyncConstructObject::GetFactoryFunction() const
{
...
TMap<FString, FAsyncTaskPinRedirectMapInfo> UK2Node_BaseAsyncConstructObject::AsyncTaskPinRedirectMap;
bool UK2Node_BaseAsyncConstructObject::bAsyncTaskPinRedirectMapInitialized = false;


const FString& UK2Node_BaseAsyncConstructObject::FHelper::GetAsyncTaskProxyName()
{
    static const FString Name(TEXT("AsyncTaskProxy"));
    return Name;
}

bool UK2Node_BaseAsyncConstructObject::FHelper::ValidDataPin(const UEdGraphPin* Pin, EEdGraphPinDirection Direction, const UEdGraphSchema_K2* Schema)
{
    check(Schema);
...
bool UK2Node_BaseAsyncConstructObject::FHelper::CreateDelegateForNewFunction(UEdGraphPin* DelegateInputPin, FName FunctionName, UK2Node* CurrentNode, UEdGraph* SourceGraph, FKismetCompilerContext& CompilerContext)
{
    const UEdGraphSchema_K2* Schema = CompilerContext.GetSchema();
...
bool UK2Node_BaseAsyncConstructObject::FHelper::CopyEventSignature(UK2Node_CustomEvent* CENode, UFunction* Function, const UEdGraphSchema_K2* Schema)
{
...
bool UK2Node_BaseAsyncConstructObject::FHelper::HandleDelegateImplementation(
